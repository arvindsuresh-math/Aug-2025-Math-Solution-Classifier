def solve(
        ...
        hours_per_12_hour_shift: int = 12, # and 1 12-hour shift
        average_tips_per_hour: int = 12, # He also makes an average of $12 in tips each hour
        tax_rate: float = 0.20, # Brendan is supposed to pay 20% of his income in taxes
        fraction_tips_reported: float = 1/3 # he only reports 1/3rd of his tips
    ):
    """Code for Q 6237 from the GSM8K dataset (train).
    Returns the total amount of money Brendan pays in taxes each week.
    """
    # First find the total number of hours Brendan works for the 8-hour shifts: 2 shifts * 8 hours/shift = <<2*8=16>>16 hours
    hours_8_hour_shifts = num_8_hour_shifts * hours_per_8_hour_shift
    ...
    total_hours_worked = hours_8_hour_shifts + hours_per_12_hour_shift
    ...
    total_tip_earnings = average_tips_per_hour * total_hours_worked

    # Now divide that number by 3, since Brendan only reports a third of his tips: $336/week / 3 = $<<336/3=112>>112/week
    reported_tips = total_tip_earnings * fraction_tips_reported
    ...
    # The final answer is the amount of taxes paid
    return taxes_paid