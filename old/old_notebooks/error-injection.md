...
def reconstruct_solution_lines(logical_steps: list[dict], eval_trace: dict[str, Any]) -> dict[str, str] | None:
    ...
    for step in logical_steps:
        ...
        try:
            reconstructed_mapping[ln] = template.format_map(eval_trace)
...
def generate_training_artifacts(logical_steps: list[dict], error_details: dict[str, Any], flawed_trace: dict[str, Any]):
    ...
    base_explanation = f"The result of this computation should be {error_details['correct_value']}, not {error_details['flawed_value']}."
...
def generate_all_valid_errors(
    ...
    This version includes a retry loop to increase the yield of valid errors,
    especially for types that are frequently filtered out by validation.
    """
    ...
    for variable_name in target_variables:
        ...
        for generator_func in applicable_generators:
            # --- Retry Loop ---
            # Attempt to generate a valid error up to 'max_attempts' times.
            for attempt in range(max_attempts):
                # Create a unique, reproducible seed for each attempt.
                ...
                # If the trace is valid, we have succeeded.
                if flawed_trace and is_trace_valid(flawed_trace, correct_trace):
                    ...
                    if artifacts:
                        ...
                        # --- Break the retry loop on first success ---
                        # We only need one valid example per (variable, error_type) pair.
                        break
            # End of the retry loop for this generator_func
        # End of the loop for all generators on this variable
    # End of the loop for all variables
...
```python
def generate_errors_for_all_templates():
    """Drives the entire error generation pipeline for all processed templates."""
    print("--- Starting Error Generation for All Processed Templates ---")
    all_metadata = []
    ...
    for tier_dir in tqdm(tier_dirs, desc="Tiers"):
        ...
        for index_dir in tqdm(index_dirs, desc=f"Processing {tier_dir.name}", leave=False):
            ...
            for model_name in MODELS:
                ...
                if not (solve_module and logical_steps): continue
                
                solve_function = solve_module.solve
                correct_trace = execution_trace(solve_function)
                
                if not correct_trace:
                    all_metadata.append({"index": index, "tier": tier_dir.name, "model": model_name, "correct_trace_generated": False})
                    continue
                
                valid_errors = generate_all_valid_errors(solve_function, logical_steps, correct_trace)
                if valid_errors:
                    records = save_error_artifacts(tier_dir.name, index, model_name, valid_errors)
                    for record in records: record['correct_trace_generated'] = True
                    all_metadata.extend(records)
                else:
                    all_metadata.append({"index": index, "tier": tier_dir.name, "model": model_name, "correct_trace_generated": True, "error_type": "NoValidErrorsFound"})
    ...
    if all_metadata:
        ...
        catalog_df = catalog_df.reindex(columns=cols)
        ...
        print("\nNo templates were found to process.")
    print("--- Error Generation Pipeline Complete ---")
```

### Cell 9: Execute the Pipeline


```python
generate_errors_for_all_templates()
```

    --- Starting Error Generation for All Processed Templates ---
    ...
    Catalog with 15529 records saved to: /Users/arvindsuresh/Documents/Github/Erdos-DL-June25-Math/data/computational-errors-generated/computational_error_catalog.csv
...
```
    ...
    --- Error Type Counts ---
    error_type
    generate_digit_transposition_error             6638
    generate_off_by_n_error                        5031
    generate_off_by_factor_of_10_error             1783
    generate_stem_off_by_n_error                   1109
    NoValidErrorsFound                              346
    generate_multiplication_by_reciprocal_error     334
