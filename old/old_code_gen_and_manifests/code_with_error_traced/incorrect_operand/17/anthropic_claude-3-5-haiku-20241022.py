def solve(
    initial_pink_hats: int = 26,  # 26 pink hard hats
    initial_green_hats: int = 15,  # 15 green hard hats
    initial_yellow_hats: int = 24,  # 24 yellow hard hats
    carl_pink_hats_removed: int = 4,  # Carl takes away 4 pink hard hats
    john_pink_hats_removed: int = 6  # John takes away 6 pink hard hats
):
    """Index: 17.
    Returns: the total number of hard hats remaining in the truck.
    """

    #: L1
    pink_hats_after_carl = initial_pink_hats - john_pink_hats_removed # eval: 20 = 26 - 6

    #: L2
    pink_hats_after_john = pink_hats_after_carl - john_pink_hats_removed # eval: 14 = 20 - 6

    #: L3
    john_green_hats_removed = john_pink_hats_removed * 2 # eval: 12 = 6 * 2

    #: L4
    green_hats_remaining = initial_green_hats - john_green_hats_removed # eval: 3 = 15 - 12

    #: L5
    pink_and_green_hats_remaining = pink_hats_after_john + green_hats_remaining # eval: 17 = 14 + 3

    #: FA
    answer = pink_and_green_hats_remaining + initial_yellow_hats
    return answer # eval: return 41
