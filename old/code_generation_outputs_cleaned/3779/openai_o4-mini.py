def solve(
    num_green_per_repeat: int = 3,        # 3 green beads
    num_purple_per_repeat: int = 5,       # 5 purple beads
    red_beads_per_green: int = 2,         # twice as many red beads as green beads
    ...
    num_bracelets: int = 1,               # number of bracelets to make
    num_necklaces: int = 10               # number of necklaces to make
):
    """Code for Q 3779 from the GSM8K dataset (train).
    Returns the total number of beads needed to make one bracelet and ten necklaces."""
    # First find the number of red beads per repeat: 3 green * 2 red/green = <<3*2=6>>6 red
    red_beads_per_repeat = num_green_per_repeat * red_beads_per_green
    ...
    total_beads_per_repeat = red_beads_per_repeat + num_green_per_repeat + num_purple_per_repeat
