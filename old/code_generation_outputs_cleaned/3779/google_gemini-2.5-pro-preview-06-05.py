def solve(
    num_green_beads: int = 3, # a repeating pattern that has 3 green beads
    num_purple_beads: int = 5, # 5 purple beads
    red_beads_multiplier: int = 2, # twice as many red beads as green beads
    repeats_per_bracelet: int = 3, # the pattern repeats three times per bracelet
    repeats_per_necklace: int = 5, # and 5 times per necklace
    num_bracelets: int = 1, # to make 1 bracelets
    num_necklaces: int = 10 # and 10 necklaces
):
    """Code for Q 3779 from the GSM8K dataset (train).
    Returns the total number of beads needed to make the specified jewelry.
    """
    # First find the number of red beads per repeat: 3 green * 2 red/green = <<3*2=6>>6 red
    num_red_beads = num_green_beads * red_beads_multiplier
    ...
    beads_per_repeat = num_red_beads + num_green_beads + num_purple_beads
    ...
    total_beads_for_necklaces = beads_per_necklace * num_necklaces
    
    # The number of beads for the bracelets is the number of beads per bracelet times the number of bracelets.
    total_beads_for_bracelets = beads_per_bracelet * num_bracelets
    ...
    total_beads = total_beads_for_necklaces + total_beads_for_bracelets
