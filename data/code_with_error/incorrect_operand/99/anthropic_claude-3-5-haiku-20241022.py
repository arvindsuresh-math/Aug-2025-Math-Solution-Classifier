def solve(
    first_tank_water: int = 48,  # There are 48 gallons of water in the first tank
    fish_size_first_tank: int = 3,  # three-inch fish in the first tank
    fish_size_second_tank: int = 2  # two-inch fish in the second tank
):
    """Index: 99.
    Returns: the number of more fish Gail would have in the first tank than the second tank after one fish eats another.
    """

    #: L1
    second_tank_water = fish_size_first_tank / 2

    #: L2
    fish_in_second_tank = second_tank_water / fish_size_second_tank

    #: L3
    fish_in_first_tank = first_tank_water / fish_size_first_tank

    #: L4
    fish_in_first_tank_after_eating = fish_in_first_tank - 1

    #: L5
    difference_in_fish = fish_in_first_tank_after_eating - fish_in_second_tank

    #: FA
    answer = difference_in_fish
    return answer