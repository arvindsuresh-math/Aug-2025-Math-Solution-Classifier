def solve(
        num_people: int = 4,  # Ann, Bill, Cate, and Dale
        pieces_per_pizza: int = 4,  # personal pan pizzas cut into 4 pieces
        bill_dale_percent_eaten: float = 0.5,  # Bill and Dale eat 50% of their pizzas
        ann_cate_percent_eaten: float = 0.75  # Ann and Cate eat 75% of the pizzas
    ):
    """Index: 30.
    Returns: the number of pizza pieces left uneaten.
    """
    #: L1
    total_pizza_pieces = num_people * pieces_per_pizza

    #: L2
    bill_dale_pieces_eaten = 2 * pieces_per_pizza * bill_dale_percent_eaten

    #: L3
    ann_cate_pieces_eaten = 2 * pieces_per_pizza * ann_cate_percent_eaten

    #: L4
    total_pieces_eaten = bill_dale_pieces_eaten + ann_cate_pieces_eaten

    #: L5
    pieces_uneaten = total_pizza_pieces - total_pieces_eaten

    answer = pieces_uneaten  # FINAL ANSWER
    return answer